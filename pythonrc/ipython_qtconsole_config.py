...
# The name of the IPython directory. This directory is used for
# logging configuration (through profiles), history storage, etc. The
# default is usually $HOME/.ipython. This options can also be
# specified through the environment variable IPYTHON_DIR.
# c.IPythonQtConsoleApp.ipython_dir = u'/home/massimo/.ipython'
...
# default is usually $HOME/.ipython. This options can also be
# specified through the environment variable IPYTHON_DIR.
# c.IPKernelApp.ipython_dir = u'/home/massimo/.ipython'
...
c.IPythonWidget.editor_line = u'emacsclient +{line} +{filename}'
...
c.IPythonWidget.editor = 'emacsclient {filename}'
...
# replaces the reload() function, so you don't need to change anything
# to use it). deep_reload() forces a full reload of modules whose code
...
#
c.ZMQInteractiveShell.readline_parse_and_bind = [
    'tab: complete',
    '"\\C-l": clear-screen',
...
# change it permanently in your config file.  Setting it to 0
# completely disables the caching system, and the minimum value
# accepted is 20 (if you provide a value less than 20, it is reset to
# 0 and a warning is issued).  This limit is defined because otherwise
...
#
# The Session object handles building messages and sending them with
# ZMQ sockets or ZMQStream objects.  Objects can communicate with each
# other over the network via Session objects, and only need to work
# with the dict-based IPython message spec. The Session will handle
